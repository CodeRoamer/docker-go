...
// Struct to Map
func ParseStruct2Map(opts interface{}) map[string]string {

	var ret_map = make(map[string]string)
	...
	for i := 0; i < value.NumField(); i++ {
		...
		case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
			if v.Int() > 0 {
				ret_map[key] = strconv.FormatInt(v.Int(), 10)
			}
		case reflect.Float32, reflect.Float64:
			if v.Float() > 0 {
				ret_map[key] = strconv.FormatFloat(v.Float(), 'f', -1, 64)
			}
		case reflect.String:
			if v.String() != "" {
				ret_map[key] = v.String()
			}
		case reflect.Ptr:
			if !v.IsNil() {
				if b, err := json.Marshal(v.Interface()); err == nil {
					ret_map[key] = key, string(b)
				}
			}
		}
	}
